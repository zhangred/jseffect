...
overtouch.prototype = {
    ...
    bindevent:function(){
        ...
        elm.addEventListener('touchstart',function(event){
            ...
            document.body.addEventListener('touchmove',_this.touchmove,{ passive: false })
...
function searchBox(opts) {
    ...
    let time = opts.time || 500
    let key = ''
    let _this = this;
    input.on('input', function(){
        key = input.val();
        if (clear) {
            key ? clear.fadeIn():clear.fadeOut()
        }
        _this.onInput()
    })

    _this.onInput = CUES.debounce(function(){
...
// 主题色控制输出
function changeThemeColor(list) {
    let themelink = document.getElementById('J_MARKETING');
    if (!themelink) {
      themelink = document.createElement('style');
      themelink.setAttribute('type', 'text/css');
      themelink.setAttribute('id', 'J_MARKETING');
    ...
    inner = `:root{${inner}}`;
    themelink.innerText = inner;
}
window.changeTheme = function () {
    let themeType = window.globalBrandTheme || 'blue';
    if (themeType === 'blue') {
      let list = [
        { key: '--primary', value: '#18AEF7' },
        { key: '--primary-light', value: '#edf9ff'},
        { key: '--gradient-start', value: '#27AEFF'},
        { key: '--gradient-end', value: '#146FF8'},
      ];
      changeThemeColor(list);
    } else if (themeType === 'red') {
      let list = [
        { key: '--primary', value: '#E14C46' },
        { key: '--primary-light', value: '#ffdddc'},
        { key: '--gradient-start', value: '#ff9d56'},
        { key: '--gradient-end', value: '#e14c46'},
      ];
      changeThemeColor(list);
    }
};
...
// 图文混编方法
function editContenteditable(opts) {
    // 可对opts 进行扩编 比如上传相关参数等
    let upfile = document.getElementById(opts.upfileId||'');
    let content = document.getElementById(opts.containerId||'');
    let holder = document.getElementById(opts.holderIdL || '')
    if (!content) {
        CUES.tip({"msg":"未设置容器id"})
        return this;
    }
    ...
    function checkValue (){
        let val = content.innerHTML||'';
        val = val.trim()
        opts.onChange && opts.onChange(val)
        if (holder) {
            holder.style.display = val ? 'none' : 'block'
        }
    }
